...
if [[ "<PROVISION_WEB_APP>" == 'false' ]]; then
    ...
    bigip1_instanceid=$(aws cloudformation describe-stacks --stack-name $bigip2_stack_name --region <REGION> | jq -r '.Stacks[].Outputs[] | select (.OutputKey=="bigIpInstanceId") | .OutputValue')
