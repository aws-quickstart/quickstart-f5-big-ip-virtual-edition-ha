...
if [[ "<PROVISION_WEB_APP>" == 'false' ]]; then
    ...
    bigip1_stack_name=$(aws cloudformation describe-stacks --stack-name $workload_stack_name --region <REGION> | jq -r '.Stacks[].Outputs[] | select (.OutputKey=="bigipInstance01") | .OutputValue')
    echo "bigip1_stack_name: $bigip1_stack_name"
    ...
    echo "bigip1_instanceid: $bigip1_instanceid"
